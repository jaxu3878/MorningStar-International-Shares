...
$(document).ready(function() {
    ...
    $('#insightspopupconfirm').hide();

    /* Popup Form */

if (document.URL.indexOf("http://mscomm.morningstar.com/internationalshares") >= 0) {
    setTimeout(function() {
            window.lightbox = lity('#insightsregopopup');
    }, 7000);
}   else {
    $('#insightsregopopup').hide();
}

    var $removeinsightspopupsection = $('#meetthemanagers');

    $removeinsightspopupsection.waypoint(function() {
        $('#insightsregopopup').hide();
    });

    /* Bar Graph */
    var $bargraph = $('#graphactivate');
    ...
    /* Form Validation */
    /* Pop Up Signup Form */
    $("#insightsPopUpSignup").validate({
        submitHandler: function() {
            $.ajax({
                type: "post",
                url: "https://s1258972516.t.eloqua.com/e/f2",
                data: $('#insightsPopUpSignup').serialize(),
                complete: function(data) {
                    $('#insightspopupreceive').hide();
                    $('#insightsreceive').hide();
                    $('#insightspopupconfirm').show();
                    setTimeout(function() {
                        lightbox.close();
                        $('#insightsregopopup').hide();
